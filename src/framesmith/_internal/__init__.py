...
US state reference data, name-affix token sets, and address-token maps.

This is the public surface of ``_internal/`` for the rest of the
package. Submodules (``regex_patterns.py``, ``unicode_maps.py``,
``us_states.py``, ``name_tokens.py``, ``address_tokens.py``) are private
— cross-directory callers import from ``framesmith._internal``.
