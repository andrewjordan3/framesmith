...
``first_non_null_per_group`` builds the aggregation expression for collapsing
duplicate-key rows into one, populating each non-key field from the first
non-null value its group offers. The caller supplies the frame method:
